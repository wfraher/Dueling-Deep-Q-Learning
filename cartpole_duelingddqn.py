...
class QNetwork:
    ...
    def __init__(self):
        ...
        self.W2 = tf.Variable(xavier([hidden_size,hidden_size]))
        self.b2 = tf.Variable(xavier([hidden_size]))
        self.dense2 = tf.matmul(self.dense, self.W2) + self.b2
        self.V, self.A = tf.split(self.dense2, 2, axis=1)
        self.VW = tf.Variable(xavier([hidden_size//2,1]))
        self.AW = tf.Variable(xavier([hidden_size//2,action_size]))
        self.VB = tf.Variable(xavier([1]))
        self.AB = tf.Variable(xavier([action_size]))
        self.Vout = tf.matmul(self.V, self.VW) + self.VB
        self.Aout = tf.matmul(self.A, self.AW) + self.AB
        self.predict = self.Vout + self.Aout - tf.reduce_max(self.Aout, axis=1, keep_dims=True)
